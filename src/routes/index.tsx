...
function Index() {
  ...
  const isExcluded = (p: Product) =>
    /hella\s+(ak|arkansas|fl\s+state|ks|spain\s+champions|jacksonville)\s+unisex(\s+zip)?\s+hood/i.test(p.title) ||
    /jacksonville\s+unisex(\s+zip)?\s+hood/i.test(p.title);
