    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          const url = new URL(request.url).searchParams.get("p") ?? "/stores";
          if (url === "sync") {
            const { syncCatalog } = await import("@/lib/printful-fulfillment.server");
            const out = await syncCatalog();
            return new Response(JSON.stringify({ ok: true, out }), { headers: { "Content-Type": "application/json" } });
          } const res = await printful(url);
